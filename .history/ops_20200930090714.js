		...
		xhr.onreadystatechange = function() 
		{
			if (this.readyState == 4 && this.status == 200) 
			{
				...
				for( var i=0; i<jsonObject.searchResults.length; i++ )
				{
					...
					// delete button 
					contactList += "<button class='btn btn-danger' type='button' onclick='deleteContact(" + jsonObject2[4] + "); onclick='reply_click_delete(this.id);' searchContact();'>Delete</button>" + "</td>";
...
function editContact(contactId){
	...
	console.log("HERE IS THE FIRST NAME: " + firstName);
