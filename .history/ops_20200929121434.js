	...
	try
	{
		xhr.onreadystatechange = function() 
		{
			if (this.readyState == 4 && this.status == 200) 
			{
				...
				var jsonObject = JSON.parse( xhr.responseText );document.getElementById("contactSearchResult").innerHTML = "Contact(s) has been retrieved";
				...
				for( var i=0; i<jsonObject.searchResults.length; i++ )
				{
					...
					// edit button
					contactList += "<td>" + "<button class='btn btn-primary mr-sm-4' type='button' data-toggle='modal' data-target='#mymodal2'>Edit</button>";
